export const getExperiencesData = async () => {
    return ([
        {
            company: 'PT Bina Bangkit Nusantara (Cakap)',
            role: 'DevOps Engineer Intern',
            start: 'Jan 2025',
            end: 'Mar 2025',
            duration: 3,
            location: 'Bandung, West Java, Indonesia',
            description: []
        },
        {
            company: 'PT. Widata Intelligent Solution',
            role: 'Software Engineer',
            start: 'Jan 2024',
            end: 'Nov 2024',
            duration: 11,
            location: 'Bandung, West Java, Indonesia',
            description: [
                'Developed and maintained 4 web applications using frameworks such as Flask, React.js, Laravel, and Tailwind.',
                'Researched, fine-tuned, and integrated pre-trained AI models into production, ensuring 100% functionality and optimizing memory usage for better resource efficiency.',
                'Collaborated with cross-functional teams of 4-6 members to deliver projects on time, utilizing effective communication and teamwork to meet client requirements.',
            ],
        },
        {
            company: 'Universitas Pendidikan Indonesia',
            role: 'Data Mining and Warehouse Course Practicum Assistant',
            start: 'Sep 2023',
            end: 'Jan 2024',
            duration: 5,
            location: 'Bandung, West Java, Indonesia',
            description: [
                'Led practicum sessions for 80+ students in understanding data mining concepts through bi-weekly classes, improving student proficiency and hands-on skills by 20% through personalized guidance.',
                'Collaborated with a team of 5 assistants to organize and deliver class materials, achieving a 90% course completion rate with improved student performance in practical assignments.',
            ],
        },
        {
            company: 'Universitas Pendidikan Indonesia',
            role: 'Database System Course Practicum Assistant',
            start: 'Feb 2023',
            end: 'Jun 2023',
            duration: 5,
            location: 'Bandung, West Java, Indonesia',
            description: [
                'Led practicum sessions for 80+ students in understanding database concepts and emphasizing SQL techniques through bi-weekly classes, increasing class participation rates to over 92% through interactive discussions and hands-on activities.',
                'Collaborated with a team of 6 assistants to organize and deliver class materials, achieving a 90% course completion rate with substantial improvement in students understanding of database normalization and SQL techniques.',
            ],
        },
    ]);
};